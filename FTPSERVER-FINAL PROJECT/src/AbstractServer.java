...
// When a connection attempt occurs it creates a new ConnectionToClient instance which runs as a thread.
{
  /**
...
// CONSTRUCTOR ******************************************************
   ...
   */
  public AbstractServer(int port) //every client has his own thread
  {
    this.port = port;
...
// INSTANCE METHODS *************************************************
  ...
  final public void listen() throws IOException
  {
    if (!isListening())
    {
      ...
      {
        serverSocket = new ServerSocket(getPort(), backlog); //create new Server Socket, biểu thị số hiệu cổng (port) nào để xuất hiện giao tiếp.
      }

      serverSocket.setSoTimeout(timeout);// it means that if no data arrives within 500ms a SocketTimeoutException will be thrown
      readyToStop = false;//Indicates if the listening thread is ready to stop.
      connectionListener = new Thread(this);//create a new Thread based on this class, pass the abstract server
  ...
  public void sendToAllClients(Object msg)
  {
    ...
    for (int i=0; i<clientThreadList.length; i++)
    {
      try
      {
        ...
        //method sendToClient is from ConnectionToClient
      }
      catch (Exception ex) {}
    }
  }
...
// RUN METHOD -------------------------------------------------------
  ...
  {
    // call the hook method to notify that the server starts listening for connections
    serverStarted();
    ...
    try
    {
      ...
      while(!readyToStop)
      {
        try
        {
          ...
          //If the server successfully binds to its port, then the ServerSocket object is successfully created
          //and the server continues to the next step—accepting a connection from a client (the next statement in the try-with-resources statement):
...
// METHODS TO BE USED FROM WITHIN THE FRAMEWORK ONLY ----------------
  ...
  final synchronized void receiveMessageFromClient(
    Object msg, ConnectionToClient client)
  {
    this.handleMessageFromClient(msg, client); //here is the message coming from client, what do you want to do with it?
  }
}
